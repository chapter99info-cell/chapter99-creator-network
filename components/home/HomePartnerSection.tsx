...
export function HomePartnerSection() {
  ...
  return (
    <section className="bg-white px-6 py-32">
      <div
        ref={containerRef}
        className="relative mx-auto max-w-7xl overflow-hidden rounded-[40px] bg-surface px-6 py-24 text-center sm:px-12"
      >
        {thumbs.map((thumb) => (
          <div
            key={thumb.id}
            className="pointer-events-none absolute h-16 w-20 animate-thumb-fade rounded-lg bg-white border border-gray-200"
            style={{
              left: thumb.x - 40,
              top: thumb.y - 32,
        ...
        <h2 className="font-heading text-5xl font-semibold leading-tight text-gray-900 md:text-8xl">
          ร่วมเป็น
          <br />
          ส่วนหนึ่งของ
          <br />
          <span className="text-trust">Creator Network</span>
        </h2>
        ...
        <Link
          href="/join"
          className="mt-10 inline-block rounded-full bg-trust px-10 py-4 text-sm font-medium text-gray-900 transition-colors hover:bg-trust/90"
