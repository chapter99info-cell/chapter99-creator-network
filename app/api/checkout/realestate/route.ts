...
import { z } from 'zod'
import { createRealEstateSubscriptionCheckout } from '@/lib/stripe-property-subscriptions'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const session = await createRealEstateSubscriptionCheckout({
      email: input.email,
    ...
    const message = isStripeAuth
      ...
      : raw
    if (isStripeAuth) {
      console.error('[checkout/realestate] Stripe configuration error')
    } else {
      console.error('[checkout/realestate]', raw)
    }
